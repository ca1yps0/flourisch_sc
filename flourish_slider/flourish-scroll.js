...
function addToConsole(message, value) {
  $('.fconsole').append("<p><b>"+message+"</b> "+value+"</p><br>");
}
...
function checkDelay() {
  ...
  if(customeDelayOverlay ) {
    ...
    if(slideNumber == noSlideDelay) {
       ...
       addToConsole('delay test-scroll');
      // for hover iframe graphic
        $(".test-scroll" ).fadeOut();
        _.delay( function () {
           console.log('fadding in');
           addToConsole('fadding in');
          $(".test-scroll").fadeIn();
        }, delayOverlay);
    }
  }
...
// device detection
  if(/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|ipad|iris|kindle|Android|Silk|lge |maemo|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pcket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino/i.test(navigator.userAgent)
...
*/
    ...
    $('.test-scroll').on('mousewheel',callbackscrll);
